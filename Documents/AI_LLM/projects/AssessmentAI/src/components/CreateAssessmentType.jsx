...
import { Plus, X, FileText, Brain, Code, Users, Globe, Layers, Zap, Database, Settings, BookOpen } from 'lucide-react'
import { assessmentTypesService } from '../services/database'
import { useAuth } from '../contexts/AuthContext'

const CreateAssessmentType = ({ onAssessmentCreated, onClose }) => {
  const [name, setName] = useState('')
  const [description, setDescription] = useState('')
  const [selectedIcon, setSelectedIcon] = useState('FileText')
  const [selectedColor, setSelectedColor] = useState('#f6d55c')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data, error } = await assessmentTypesService.createAssessmentType(user.id, {
        name: name.trim(),
